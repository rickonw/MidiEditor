...
MidiFile::MidiFile(){
	...
	channel(18)->eventMap()->insert(make_pair(0, timeSig));

	// create tempo change
	TempoChangeEvent *tempoEv = new TempoChangeEvent(17, 500000, tempoTrack);
	tempoEv->setFile(this);
	channel(17)->eventMap()->insert(make_pair(0, tempoEv);

	playerMap = new multimap<int, MidiEvent*>;

	midiTicks = 7680;
	calcMaxTime();
}

MidiFile::MidiFile(string path, bool *ok, string *log) {
	...
	ifstream *stream = new ifstream(f);
	//stream->setByteOrder(ifstream::BigEndian);
...
bool MidiFile::readMidiFile(ifstream *content, string *log){
	...
	string badHeader = "Error: Bad format in file header (Expected MThd).";
	(*content)>>tempByte;
	...
	for(int num = 0; num<numTracks; num++){
		...
		if(!ok){
			log->append("Error in Track "+to_string(num));
			return false;
		}
	}
    ...
    Qlist<OnEvent *>::iterator iter;
	OnEvent *event;
	Qlist<OnEvent*> _events = OffEvent::corruptedOnEvents();
	for (Qlist<OnEvent*>::iterator iter = _events.begin(); iter != _events.end(); iter++) {
		OnEvent *onevent = dynamic_cast<OnEvent*>(*iter);
		log->append("Warning: found OnEvent without OffEvent (line "+to_string(onevent->line()) +") - removing...");
...
bool MidiFile::readTrack(ifstream *content, int num, string *log){

	quint8 tempByte;

	string badHeader = "Error: Bad format in track header (Track "+to_string(num)+", Expected MTrk).";
	(*content)>>tempByte;
	if(tempByte != 'M'){ log->append(badHeader); return false; }
	(*content)>>tempByte;
	if(tempByte != 'T'){ log->append(badHeader); return false; }
	...
	// end of track
	(*content)>>tempByte;
	string errorText = "Error: track " + to_string(num) +"not ended as expected. ";
...
int MidiFile::deltaTime(ifstream *content){
	return variableLengthvalue(content);
}


int MidiFile::variableLengthvalue(ifstream *content){
	quint32 v = 0;
	quint8 byte = 0;
	do {
		(*content)>>byte;
...
map<int, MidiEvent*> *MidiFile::timeSignatureEvents(){
	return channels[18]->eventMap();
}

map<int, MidiEvent*> *MidiFile::tempoEvents(){
...
int MidiFile::tick(int startms, int endms, Qlist<MidiEvent*> **eventList,
	...
	*msOfFirstEvent = time;
	...
	// get the endTick
	for(; i<events.max_size() && timeMsNextEvent<endms; i++){
...
multimap<int, MidiEvent*> *MidiFile::channelEvents(int channel){
...
string MidiFile::instrumentName(int prog){
	switch(prog+1){
...
string MidiFile::controlChangeName(int control){

	switch(control){
		case 0: { return "Bank Select (MSB)"; }
		case 1: { return "Modulation Wheel (MSB)"; }
		...
		case 79: { return "Sound Controller 10 (GM2 default: Undefined)"; }
		...
		case 91: { return "Effects 1 Depth (default: Reverb Send)"; }
		case 92: { return "Effects 2 Depth (default: Tremolo Depth)"; }
		case 93: { return "Effects 3 Depth (default: Chorus Send)"; }
		case 94: { return "Effects 4 Depth (default: Celeste [Detune] Depth)"; }
		case 95: { return "Effects 5 Depth (default: Phaser Depth)"; }
...
Qlist<MidiEvent*> *MidiFile::eventsBetween(int start, int end){
	...
	for(int i = 0; i<19; i++){
		multimap<int, MidiEvent*> *events = channels[i]->eventMap();
		multimap<int, MidiEvent*>::iterator current=events->lowerBound(start);
		multimap<int, MidiEvent*>::iterator upper_bound=events->upper_bound(end);
		while (current != upper_bound) {
			if(!eventList->contains(current.value())) {
				eventList->push_back(current.value());
			}
			current++;
		}
	}
...
void MidiFile::preparePlayerData(int tickFrom){
	...
	for(int i = 0; i<19; i++){
		...
		multimap<int, MidiEvent*> *channelEvents = channels[i]->eventMap();
		multimap<int, MidiEvent*>::iterator it=channelEvents->begin();
		...
		while(it!=channelEvents->end()){
			...
			if(tick>=tickFrom){
				...
				if(!event->track()->muted()){
					playerMap->insert(make_pair(ms, event));
				}
			} else {
		...
		if(prgList->size()>0){
			...
			playerMap->insert(make_pair(msOfTick(tickFrom)-1, prgList->back()));
		}
...
multimap<int, MidiEvent*> *MidiFile::playerData(){
	return playerMap;
}
...
bool MidiFile::save(string path){
	...
	multimap<int, MidiEvent*> allEvents = multimap<int, MidiEvent*>();
	for(int i = 0; i<19; i++){
		multimap<int, MidiEvent*>::iterator it=channels[i]->eventMap()->begin();
		while(it!=channels[i]->eventMap()->end()){
			allEvents.insert(make_pair(it.key(), it.value()));
			it++;
		}
	}
	...
	for(int num = 0; num<numTracks(); num++){
		...
		int numBytes = 0;
		int currentTick = 0;
		multimap<int, MidiEvent*>::iterator it=allEvents.begin();
		while(it!=allEvents.end()){
			...
			if(_tracks->at(num) == event->track()){
				...
				ByteArray deltaTime = writeDeltaTime(time);
				numBytes += deltaTime.count();
				data.append(deltaTime);

				// write the events data
				ByteArray eventData = event->save();
...
ByteArray MidiFile::writeDeltaTime(int time){
	return writeVariableLengthValue(time);
}

ByteArray MidiFile::writeVariableLengthValue(int value){
...
bool MidiFile::removeTrack(MidiTrack *track){
	...
	foreach(MidiFile *fileFrom, pasteTracks.keys()){
		...
		map<MidiTrack*, MidiTrack*> tracks = pasteTracks.value(fileFrom);
		foreach(MidiTrack *source, sourcesToRemove){
            map<MidiTrack*,MidiTrack*>::iterator iter;
		    iter=tracks.find(source);
            tracks.erase(iter);
			//tracks.remove(source);
		}
		pasteTracks.insert(make_pair(fileFrom, tracks));
	}
	int n = 0;
	foreach(MidiTrack *track, *_tracks){
		track->setNumber(n++);
...
int MidiFile::tonalityAt(int tick){
	multimap<int, MidiEvent*> *events = channels[16]->eventMap();

	multimap<int, MidiEvent*>::iterator it = events->begin();
...
void MidiFile::meterAt(int tick, int *num, int *denum){
	map<int, MidiEvent*> *meterEvents = timeSignatureEvents();
	map<int, MidiEvent*>::iterator it = meterEvents->begin();
	TimeSignatureEvent *event = 0;
...
void MidiFile::printLog(string *log){
	foreach(string str, *log){
...
//	}
	...
	map<MidiTrack*, MidiTrack*> Qlist;
	if(pasteTracks.contains(fileFrom)){
		Qlist = pasteTracks.value(fileFrom);
	}

	Qlist.insert(make_pair(source, destination));
	pasteTracks.insert(make_pair(fileFrom, Qlist));
